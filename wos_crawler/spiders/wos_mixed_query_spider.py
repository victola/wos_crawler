...
class WosMixedQuerySpiderSpider(scrapy.Spider):
    name = 'wos_mixed_query_spider'
    ...
    def __init__(self, query = None, journal_list_path = None, output_path = '../output', document_type='Article',output_format = 'bibtex',gui=None, *args, **kwargs):
    ...
    def parse(self, response):
        ...
        if len(self.JOURNAL_LIST) > 0:
            query = " OR ".join(self.JOURNAL_LIST)
            journal_query = 'SO=({})'.format(query.upper())
            self.query = self.query + ' AND ' + journal_query
        else:
            print('**期刊列表为空**')
